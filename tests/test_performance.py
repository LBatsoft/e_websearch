...
class PerformanceTester:
    ...
    async def test_cache_performance(self):
        ...
        print(f"✅ 性能提升: {((first_search_time - cached_search_time) / first_search_time * 100):.1f}%")
        
        return {
            "first_search_time": first_search_time,
            "cached_search_time": cached_search_time,
            "performance_improvement": ((first_search_time - cached_search_time) / first_search_time * 100)
        }
    ...
    async def test_cache_stats(self):
        ...
        if cache_stats.get('type') == 'distributed':
            ...
            print(f"✅ 本地缓存大小: {local_stats.get('current_size', 0)}/{local_stats.get('max_size', 0)}")
        ...
        try:
            async with aiohttp.ClientSession() as session:
                ...
                if search_times:
                    ...
                    print(f"✅ API缓存命中率: {cache_hits}/{len(results)} ({cache_hits/len(results)*100:.1f}%)")
                
                return {
                    "total_time": total_time,
                    "avg_response_time": statistics.mean(search_times) if search_times else 0,
    ...
    async def run_all_tests(self):
        ...
        # 测试并发性能
        try:
            results["concurrent_performance"] = await self.test_concurrent_performance(5)
            print()
        except Exception as e:
            print(f"❌ 并发性能测试失败: {e}")
