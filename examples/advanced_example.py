...
async def multi_source_search():
    ...
    for query in queries:
        ...
        print(
            f"找到 {len(response.results)} 个结果，耗时 {response.execution_time:.2f}秒"
        )
...
async def export_results_example():
    ...
    request = SearchRequest(
        query="深度学习框架", max_results=5, sources=[SourceType.BING]
    )
    ...
    for result in response.results:
        result_dict = {
            ...
            "publish_time": (
                result.publish_time.isoformat() if result.publish_time else None
            ),
