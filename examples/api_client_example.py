...
class EWebSearchClient:
    ...
    def __init__(self, base_url: str = "http://localhost:8000"):
        ...
        self.session.headers.update(
            {"Content-Type": "application/json", "User-Agent": "EWebSearch-Client/1.0"}
        )
    ...
    def get_suggestions(self, query: str) -> Dict[str, Any]:
        ...
        try:
            response = self.session.post(
                f"{self.base_url}/suggestions", json={"query": query}
            )
            response.raise_for_status()
            return response.json()
        except requests.exceptions.RequestException as e:
            return {"success": False, "suggestions": [], "query": query}
    ...
    def clear_cache(self) -> Dict[str, Any]:
        ...
        try:
            ...
            return {"success": False, "message": f"清空缓存失败: {e}"}
...
def demo_search_examples():
    ...
    # 2. 基础搜索
    print("\n2. 🔍 基础搜索")
    result = client.search(query="Python编程教程", max_results=3, sources=["zai"])
    ...
    result = client.search(
        ...
        max_results=5,
        sources=["zai"],
        filters={"time_range": "month", "content_size": "high"},
    )
...
def interactive_search():
    ...
    while True:
        try:
            ...
            if result["success"]:
                print(
                    f"✅ 找到 {result['total_count']} 个结果 (耗时: {result['execution_time']:.2f}秒)"
                )
...
def benchmark_test():
    ...
    test_queries = ["Python编程", "人工智能", "机器学习", "数据科学", "Web开发"]
