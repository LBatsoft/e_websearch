            ...
            async with aiohttp.ClientSession() as session:
                ...
                async with session.post(
                    ...
                    if response.status >= 400:
                        logger.error(
                            f"API 错误响应: {response.status} - {response_text}"
                        )
                        return []
                    ...
                    try:
                        ...
                        logger.error(
                            f"JSON 解析失败: {e}, 原始响应: {response_text[:200]}"
                        )
                        return []
                    ...
                    if isinstance(api_results, dict):
                        ...
                        items = (
                            api_results.get("articles", [])
                            or api_results.get("data", [])
                            or api_results.get("results", [])
                        )
                    ...
                    parsed_results = [
                        self.parse_item(item, request.query) for item in items
                    ]
                    # 过滤掉解析失败的结果 (None)
...
class WeChatSearcher(BasePrivateSearcher):
    ...
    def parse_item(self, item: Dict[str, Any], query: str) -> Optional[SearchResult]:
        try:
            ...
            logger.debug(
                f"提取的字段 - 标题: {title[:50]}, URL: {url}, 内容长度: {len(full_content)}"
            )

            if not title or not url:
                logger.warning(
                    f"跳过条目：标题或URL为空 - 标题: {title[:30]}, URL: {url[:50]}"
                )
                return None
            ...
            publish_time = (
                parse_publish_time(str(publish_time_str)) if publish_time_str else None
            )

            # 处理作者信息 - 适配微信API的account字段
            author = (
                item.get("account", "")  # 直接使用account字段
                or item.get("author")
                or item.get("nickname")
                or item.get("authorName")
                or item.get("account", {}).get("name")
            )
...
class ZhihuSearcher(BasePrivateSearcher):
    ...
    def parse_item(self, item: Dict[str, Any], query: str) -> Optional[SearchResult]:
        try:
            ...
            snippet_content = (
                item.get("excerpt", "") or item.get("summary", "") or content[:50]
            )
