...
class LLMEnhancer:
    ...
    def __init__(self):
        ...
        self.providers = {
            ...
            "baidu": BaiduProvider(
                {"api_key": BAIDU_API_KEY, "secret_key": BAIDU_SECRET_KEY}
            ),
        ...
        if self.available_providers:
            logger.info(
                f"LLM 增强器初始化成功，可用提供商: {list(self.available_providers.keys())}"
            )
        else:
    ...
    async def enhance(
        ...
        if per_result and (want_summary or want_tags) and results:
            ...
            await asyncio.gather(*[with_sem(r) for r in results[: max(0, max_items)]])
