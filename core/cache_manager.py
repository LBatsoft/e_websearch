...
class RedisCacheManager(BaseCacheManager):
    ...
    def __init__(self, config: dict):
        ...
        try:
            ...
            logger.info(
                f"使用 Redis 缓存 (连接到 redis://{REDIS_CONFIG['host']}:{REDIS_CONFIG['port']})"
            )
...
class DistributedCacheManager(BaseCacheManager):
    ...
    async def clear(self):
        """清空所有缓存"""
        await asyncio.gather(self.redis_cache.clear(), self.local_cache.clear())
        self.stats = {"hits": 0, "misses": 0, "sets": 0, "evictions": 0}
        logger.info("分布式缓存已清空")

    async def close(self):
        """关闭缓存管理器"""
        await asyncio.gather(self.redis_cache.close(), self.local_cache.close())
    ...
    async def get_stats(self) -> Dict[str, Any]:
        ...
        total_requests = self.stats["hits"] + self.stats["misses"]
        if total_requests > 0:
            combined_stats["hit_rate"] = round(
                (self.stats["hits"] / total_requests) * 100, 2
            )
        else:
    ...
    async def cleanup_expired(self):
        """清理过期缓存"""
        await asyncio.gather(
            self.redis_cache.cleanup_expired(), self.local_cache.cleanup_expired()
        )
